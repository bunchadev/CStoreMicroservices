...
namespace UserService.Tests.Data
{
    ...
    {
        public static async Task InitializeDatabaseAsync(IDbConnection connection)
        {
            string hashedPassword1 = BCrypt.Net.BCrypt.HashPassword("1232003");
            string hashedPassword2 = BCrypt.Net.BCrypt.HashPassword("1232003");
            ...
            var sqlScript = @"
                ...
                VALUES 
                    (NEWID(), 'tn0888888@gmail.com', @Password1, 'credentials', @AdminRoleId),
                    (NEWID(), 'trungvjppro1232003@gmail.com', @Password2, 'credentials', @UserRoleId);
                ";
            await connection.ExecuteAsync(sqlScript, new { Password1 = hashedPassword1, Password2 = hashedPassword2 });
        }
    }
}
